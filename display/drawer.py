...
def load_icon(filename, width=None, height=None):
    with BytesIO() as out:
        cairosvg.svg2png(url=f"icons/{filename}", write_to=out,
                         background_color="white", output_width=width, output_height=height)
        im = Image.open(out)
        im.load()
        return im
...
class DisplayDrawer:
    ...
    def _draw_plant_moisture(self, image, draw, x_pos):
        ...
        x_pos += 25
        moisture = "--" if math.isnan(
            self.data.soil_moisture) else self.data.soil_moisture
    ...
    def _draw_low_temperature(self, image, draw, x_pos):
        ...
        x_pos += 25
        low_temp = "--" if math.isnan(
            self.data.day_lowhigh[0]) else self.data.day_lowhigh[0]
    ...
    def _draw_high_temperature(self, image, draw, x_pos):
        ...
        x_pos += 25
        high_temp = "--" if math.isnan(
            self.data.day_lowhigh[1]) else self.data.day_lowhigh[1]
